...
try:
    ...
    try:
        ...
        splatting_cuda = load(
            name="splatting_cuda",
            sources=[
                os.path.join(splatting_dirname, "cuda_cpp/splatting_cuda.cpp"),
                os.path.join(splatting_dirname, "cuda_cpp/splatting.cu"),
            ],
            extra_include_paths=[
                os.path.dirname(
                    glob.glob(r'/usr/local/cuda/include/cublas_v2.h')[0]
                )
            ],
            verbose=True,
            extra_cflags=["-O3"],
        )
        print("splatting.cuda loaded successfully")
    except Exception as e:
        import warnings

        warnings.warn(
            f"splatting.cuda could not be imported nor jit compiled: {e}", ImportWarning
        )
        splatting_cuda = None
